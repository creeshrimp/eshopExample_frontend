...
function openDialog(item) {
    if (item) {
        dialog.value.productId = item._id
...
const submit = handleSubmit(async (values) => {
    ...
    if (dialog.value.productId.length === 0 && fileRecords.value.length === 0) {
        createSnackbar({
    ...
    try {
        ...
        // 如果是編輯
        if (dialog.value.productId.length > 0) {
            await apiAuth.patch('/product/' + dialog.value.productId, fd)
        } else {
        ...
        createSnackbar({
            text: t(
                dialog.value.productId.length > 0
                    ? 'adminProduct.editSuccess'
                    : 'adminProduct.newSuccess',
            ),
            snackbarProps: {
                color: 'green',
            },

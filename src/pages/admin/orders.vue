<template>
    <v-container>
        <v-row>
            ...
            <v-divider></v-divider>
            <v-col cols="12">
                <v-data-table :data="orders" :headers="headers">
                    <template #[`item.cart`]="data">
                        <ul>
                            <li v-for="item in data.item.cart" :key="item._id">{{ item.product.name }} x {{ item.quantity }}</li>
                        </ul>
                    </template>
                </v-data-table>
            </v-col>
...
const getOrders = async () => {
    try {
        ...
        console.log(error)
        createSnackbar({
            text: t('api.' + (error?.response?.data?.message || 'unknownError from admin getOrders')),
            snackbarProps: {
                color: 'red',
            },
        })
